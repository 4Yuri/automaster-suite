...
- Avvia il programma: chiede il percorso del file e quanti core usare. Produce due file nella cartella del programma:
...
def detect_hum_f0_and_strength(x_mono, sr):
...
def declip_soft(x, threshold=0.985, win=64):
    ...
    while i < n:
        ...
        b = min(n - 1, j + win)
        left = a
        while left < i and mask[left]:
            left += 1
        right = b
        while right > j and mask[right]:
            right -= 1
        if left < i and right > j:
            y[i:j + 1] = np.interp(np.arange(i, j + 1), [left, right], [y[left], y[right]])
        else:
            seg = y[i:j + 1]
...
def limiter_truepeak_stereo(x_stereo, sr, tp_target_db=-1.0, lookahead_ms=5.0, atk_ms=1.0, rel_ms=50.0):
    x = np.asarray(x_stereo, dtype=np.float64)
    if x.ndim == 1:
        x = x.reshape(-1, 1)
    n, ch = x.shape
    ...
    idxs = np.where(env > target_lin)[0]
    for i in idxs:
        j = min(env.size - 1, i + la)
        for k in range(i, j + 1):
            need[k] = min(need[k], target_lin / max(env[i], 1e-12))
    g = need.copy()
...
def resample_to(sr_target, x, sr_in):
    if sr_in == sr_target:
        return np.asarray(x, dtype=np.float64), sr_in
    try:
        g = math.gcd(int(sr_in), int(sr_target))
        up = int(sr_target // g)
        down = int(sr_in // g)
        y = resample_poly(np.asarray(x, dtype=np.float64), up, down, axis=0)
    except Exception:
        n_in = x.shape[0]
        n_out = int(round(n_in * (sr_target / float(sr_in))))
...
def downsample_to_44100(stereo, sr_in):
    return resample_to(44100, stereo, sr_in)
...
def analyze_with_oracle(analyze_audio_func, x_stereo_f64, sr, temp_tag):
    ...
    os.close(fd)
    try:
        sf.write(tmp, x_stereo_f64, sr, subtype="FLOAT", format="WAV")
...
def _worker_init(analyzer_module_path, blas_threads):
    try:
        ...
        pass
    spec = importlib.util.spec_from_file_location("_am_worker_analyzer", analyzer_module_path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    globals()["_AN_FUNC"] = getattr(mod, "analyze_audio")

def _worker_analyze(path):
    try:
        res = globals()["_AN_FUNC"](path, is_temporary=True)
        if not isinstance(res, dict):
            res = {}
        return res
    except Exception:
...
def analyze_candidates_parallel(candidates, sr, tag_prefix, analyze_audio_func, analyzer_module_path, workers):
    tmp_paths = []
    results = []
    try:
        for i, y in enumerate(candidates):
            fd, tmp = tempfile.mkstemp(prefix=f"_am_{tag_prefix}_{i}_", suffix=".wav")
            os.close(fd)
            sf.write(tmp, y, sr, subtype="FLOAT", format="WAV")
            tmp_paths.append(tmp)
        if analyze_audio_func is None:
            return [{} for _ in tmp_paths], tmp_paths
        if workers and workers > 1 and analyzer_module_path:
            blas_threads = 1
            with ProcessPoolExecutor(max_workers=workers, initializer=_worker_init, initargs=(analyzer_module_path, blas_threads)) as ex:
                results = list(ex.map(_worker_analyze, tmp_paths))
        else:
            for tmp in tmp_paths:
                try:
                    ...
                    res = {}
                results.append(res)
        return results, tmp_paths
    except Exception:
        return [{} for _ in candidates], tmp_paths
...
def try_hum_notch(analyze_audio_func, analyzer_module_path, workers, base_res, Lw, Rw, sr):
    ...
    if f0 is None or score < 4.0:
        return Lw, Rw
    candidates_h = [6, 8, 12, 16]
    sos_list = []
    for nh in candidates_h:
        sos = design_hum_notch_comb(sr, f0, max_harm=nh)
        if sos is not None:
            sos_list.append((nh, sos))
    if not sos_list:
        return Lw, Rw
    cand_audio = []
    for nh, sos in sos_list:
        Lc = apply_sos_chain_filtfilt(Lw, sos)
        Rc = apply_sos_chain_filtfilt(Rw, sos)
        cand_audio.append(np.column_stack([Lc, Rc]))
    results, tmp_paths = analyze_candidates_parallel(cand_audio, sr, "hum", analyze_audio_func, analyzer_module_path, workers)
    for p in tmp_paths:
        try:
            os.remove(p)
        except Exception:
            pass
    spur_old = getf(base_res, "noise_spur_db", 1e9)
    isp_old = getf(base_res, "isp_margin_db", 0.0)
    plr_old = getf(base_res, "plr_effective_db", getf(base_res, "plr_est", 0.0))
    dr_old = getf(base_res, "dr_tt_avg", 0.0)
    best_idx = -1
    best_spur = spur_old
    for i, res in enumerate(results):
        ...
        if (spur_new <= spur_old - 3.0) and (isp_new >= isp_old - 0.2) and (plr_new >= plr_old - 0.4) and (dr_new >= dr_old - 0.4):
            if spur_new < best_spur:
                best_spur = spur_new
                best_idx = i
    if best_idx >= 0:
        return cand_audio[best_idx][:, 0], cand_audio[best_idx][:, 1]
    return Lw, Rw
...
def try_declip(analyze_audio_func, analyzer_module_path, workers, base_res, Lw, Rw, sr):
    def getf(d, k, dv):
        try:
            ...
            return dv
    tp = true_peak_estimate_stereo(np.column_stack([Lw, Rw]), sr, os_factor=8)
    if tp <= 0.0:
        return Lw, Rw
    cands = []
    for th in (0.985, 0.975):
        Lc = declip_soft(Lw, threshold=th, win=64)
        Rc = declip_soft(Rw, threshold=th, win=64)
        y = limiter_truepeak_stereo(np.column_stack([Lc, Rc]), sr, tp_target_db=-1.2)
        cands.append(y)
    results, tmp_paths = analyze_candidates_parallel(cands, sr, "declip", analyze_audio_func, analyzer_module_path, workers)
    for p in tmp_paths:
        try:
            os.remove(p)
        except Exception:
    ...
    for i, res in enumerate(results):
        ...
        gain = (isp_new - isp_old) + max(0.0, (tp_old - tp_new))
...
def try_deesser(analyze_audio_func, analyzer_module_path, workers, base_res, Lw, Rw, sr):
    ...
    depths = (1.5, 2.5)
    cand_audio = []
    for depth_db in depths:
        Lc = deesser_dynamic(Lw, sr, depth_db=depth_db, sens=1.6)
        Rc = deesser_dynamic(Rw, sr, depth_db=depth_db, sens=1.6)
        cand_audio.append(np.column_stack([Lc, Rc]))
    results, tmp_paths = analyze_candidates_parallel(cand_audio, sr, "deesser", analyze_audio_func, analyzer_module_path, workers)
    for p in tmp_paths:
        try:
            os.remove(p)
        except Exception:
            pass
    ton_old = getf(base_res, "spectral_balance_dev_db", 9.9)
    plr_old = getf(base_res, "plr_effective_db", getf(base_res, "plr_est", 0.0))
    best_idx = -1
    best_ton = ton_old
    for i, res in enumerate(results):
        ...
        plr_new = getf(res, "plr_effective_db", plr_old)
        if (ton_new <= ton_old - 0.2) and (plr_new >= plr_old - 0.3):
            if ton_new < best_ton:
                best_ton = ton_new
                best_idx = i
    if best_idx >= 0:
...
def try_tilt(analyze_audio_func, base_res, Lw, Rw, sr):
    def getf(d, k, dv):
        try:
            ...
            return dv
    Lc, Rc = gentle_tilt_if_needed(Lw, Rw, sr)
    if np.allclose(Lc, Lw) and np.allclose(Rc, Rw):
        return Lw, Rw
    res, tmp = analyze_with_oracle(analyze_audio_func, np.column_stack([Lc, Rc]), sr, "tilt")
    try:
        ton_old = getf(base_res, "spectral_balance_dev_db", 9.9)
        ton_new = getf(res, "spectral_balance_dev_db", ton_old)
        if ton_new <= ton_old - 0.2:
            return Lc, Rc
        return Lw, Rw
    finally:
        if tmp:
...
def try_stereo_width(analyze_audio_func, base_res, Lw, Rw, sr):
    def getf(d, k, dv):
        try:
            ...
            return dv
    wid_old = getf(base_res, "stereo_width_iqr_db", 0.0)
    if wid_old <= 0.0:
        return Lw, Rw
    Lc, Rc = ms_width_adjust(Lw, Rw, target_db_range=(-10.0, +6.0), max_gain_change=0.10)
    if np.allclose(Lc, Lw) and np.allclose(Rc, Rw):
        return Lw, Rw
    res, tmp = analyze_with_oracle(analyze_audio_func, np.column_stack([Lc, Rc]), sr, "stereo")
    try:
        wid_new = getf(res, "stereo_width_iqr_db", wid_old)
        if abs(wid_new - 10.0) < abs(wid_old - 10.0):
            return Lc, Rc
        return Lw, Rw
    finally:
        if tmp:
...
def prompt_workers():
    ...
    while True:
        try:
            s = input(f"Quanti core vuoi usare? [1–{maxc}] (Invio={hint}): ").strip()
        except (EOFError, KeyboardInterrupt):
            print("\nUscita.")
            sys.exit(0)
        if s == "":
...
def main():
    ...
    try:
        out_hr, out_cd = automaster_process(in_path, verbose=True, workers=workers)
        print("Operazione completata.")
        print(f" • Versione Hi‑SR (FLAC): {out_hr}")
        print(f" • Versione CD  (FLAC):   {out_cd}")
